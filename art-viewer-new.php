...
blockquote {
  font: italic 200 55px/1 'Montserrat', sans-serif;
  color: #fff;
  margin: auto;
...
<?php if (( $voteOpen ) && (!$previewPrensa)) { ?>
	...
	<?php } else { ?>
		...
		<div class="login-modal-wrapper">
          ...
          <a href="#!" class="login-btn btn" onclick="jQuery('.form-active').removeClass('form-active');jQuery(this).addClass('form-active');jQuery('.lwa-form').addClass('form-active');">            
            <span class="lang-ca">Ja tens un compte?</span>

            <span class="lang-es">¿Ya tienes cuenta?</span>
            ...
            <a href="#!" class="register-btn btn form-active" onclick="jQuery('.form-active').removeClass('form-active');jQuery(this).addClass('form-active');jQuery('.lwa-register').addClass('form-active');">

              <span class="lang-ca">No tens compte? Registra't.</span>

              <span class="lang-es">¿No tienes cuenta? Regístrate.</span>

              <span class="lang-en">Don't have an account? Register.</span>
...
<?php } else if ($previewPrensa) { ?>
	...
	<?php if ( $voteOK ) { ?>
      ...
      <p>
        <small>
          <span class="lang-ca">Accés previ</span>
          <span class="lang-es">Acceso previo</span>
          <span class="lang-en">Early access</span>
        </small>
      </p>
      ...
      <?php if ( $voteOpen ) { ?>
  			<p class="lwa-status-invalid" style="background-color: transparent; border: none;">

  				<small>

  					<span class="lang-ca">Els teus vots no es comptabilitzaran en el accès previ. Per poder votar hauràs de <?php echo '<a href="'.wp_logout_url( home_url() ).'" class="logout-link">surtir.</a>'; ?></span>

  					<span class="lang-es">Tus votos no se contabilizarán en el aceso previo. Para poder votar tendrás que <?php echo '<a href="'.wp_logout_url( home_url() ).'" class="logout-link">salir.</a>'; ?></span>

  					<span class="lang-en">Your votes will not be counted in the previous access. To be able to vote you will have to <?php echo '<a href="'.wp_logout_url( home_url() ).'" class="logout-link">leave it.</a>'; ?></span>
...
<?php } else { ?>
	...
	<div class="intro-text">
		...
		<p><span class="lang-ca">Vota el millor art de 2018. <br />A partir l'1 de gener de 2019.</span><span class="lang-es">Vota el mejor arte de 2018. <br />A partir del 1 de enero de 2019.</span><span class="lang-en">Vote for the best art of 2018. <br />Start on January 1, 2019.</span></p>
...
<blockquote>
  ...
  <span>seen in</span>
  <span>BCN'18</span>
  <span class="by-line">presented by</span>  
  <span class="sponsor"><a href="http://google.com" target="_blank">Sponsor</a></span>  
</blockquote>
...
jQuery('.lwa-links-remember').on('click tap', function(){
 jQuery('.login-btn').addClass('form-active');
});



jQuery(document).on('lwa_register', function(){
  setTimeout(formOkChecker, 500);
});


function formOkChecker() {
  if (jQuery('.lwa-status').hasClass('lwa-status-confirm')) {
    jQuery('.login-form .lwa-status').html(jQuery('.lwa-status-confirm').text());
    jQuery('.login-form .lwa-status').addClass('lwa-status-confirm copy');
    jQuery('.form-active').removeClass('form-active');
    jQuery('.login-btn, .lwa-form').addClass('form-active');
    jQuery('#lwa_wp-submit').on('click tap', function(){
      jQuery('.lwa-status-confirm.copy').removeClass('lwa-status-confirm');
    });
  }
}

jQuery('.logout-link').on('click tap', function() {
  document.cookie = 'uviews=done; expires=Thu, 01 Jan 1975 00:00:01 GMT;';
  document.cookie = 'PHPSESSID=done; expires=Thu, 01 Jan 1975 00:00:01 GMT;';
});
